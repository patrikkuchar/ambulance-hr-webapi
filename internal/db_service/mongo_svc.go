...
type DbService[DocType interface{}] interface {
	...
	FindDocumentByEmail(ctx context.Context, id string) (*DocType, error)
...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("AMBULANCE_HR_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("AMBULANCE_HR_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("AMBULANCE_HR_API_MONGODB_USERNAME", "root")
	}

	if svc.Password == "" {
		svc.Password = enviro("AMBULANCE_HR_API_MONGODB_PASSWORD", "neUhaDnes")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("AMBULANCE_HR_API_MONGODB_DATABASE", "xkucharp-ambulance-hr")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("AMBULANCE_HR_API_MONGODB_COLLECTION", "user")
	}

	if svc.Timeout == 0 {
		seconds := enviro("AMBULANCE_HR_API_MONGODB_TIMEOUT_SECONDS", "10")
...
func (this *mongoSvc[DocType]) FindDocumentByEmail(ctx context.Context, email string) (*DocType, error) {
	ctx, contextCancel := context.WithTimeout(ctx, this.Timeout)
	defer contextCancel()
	client, err := this.connect(ctx)
	if err != nil {
